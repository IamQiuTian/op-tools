...
def run_Status(ip):
    gscode = shell("adb -s {}:5555 shell dumpsys activity | grep cn.gloud.gs_android".format(ip))
    gsdcode = shell("adb -s {}:5555 shell dumpsys activity | grep cn.gloud.gs_android_dog".format(ip))
    if gscode[0] != 0:
        print(ip +"       "+ "GS not run")
        logging.error("{} not run GS")

    if gsdcode[0] != 0:
        logging.error("{} not run GSD")
        print(ip +"       "+ "GSD not run")

def check_system_version(ip):
    version = shell('adb -s {}:5555 shell getprop | grep ro.product.version'.format(ip))
    if version_tag not in str(version):
       print("{} update error".format(ip))
    else:
       print("{} update ok".format(ip))
...
if __name__ == "__main__":
    ...
    arg.add_argument('-m','--mode',help='install or version or dir or init or status or check')
    argd = arg.parse_args()
