...
class Json_code(threading.Thread):
    ...
    def __init__(self,argv):
        ...
        self.mailto_list=["xxxx@163.com","xxxx@163.com"]
    ...
    def Send_mail(self,to_list,sub,content):
        me="xxxx"+"<"+self.mail_user+"@"+self.mail_postfix+">"  
        ...
        if port_data:
            for k,v in port_data.items():
                ...
                if k and v:
                    ...
                    #日志记录格式
                    self._text = ("[{time}]  {ip} The port is not in book value, value is different {port} \n\n"
                                   .format(time=self._time,ip=k,port=v))

                    #判断日志存放目录是否存在
                    if os.path.exists("port_log"):   
                        with open("port_log/"+self._argv+"-"+self._time+".log","a") as f:
                            f.write(self._text)
                    else:
                        os.mkdir('port_log')
                    
                    #Json_code.Send_mail(self.mailto_list,"[ERR]"+self._argv+"[port error]",self._text)
